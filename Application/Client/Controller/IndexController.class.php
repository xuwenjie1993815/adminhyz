...
class IndexController extends BaseController {
    ...
    public function driver_info() {
        ...
        $info = M('driver')->alias('a')
                ...
                ->find();
//2. 截取字符串前一位
        $info['license_pic_zm'] = substr($info['license_pic_zm'], 1);
        $info['license_pic_fm'] = substr($info['license_pic_fm'], 1);
        $info['xsz_zm'] = substr($info['xsz_zm'], 1);
        $info['xsz_fm'] = substr($info['xsz_fm'], 1);
        $info['head_pic'] = substr($info['head_pic'], 1);
...
//        dump($info);exit;
    ...
    public function extend() {
        ...
        $condition['d.company_id'] = $session['id'];
        if (!empty($keywords))
            $condition['d.invite_name|d.mobile'] = ['like' => "%$keywords%"];
        $user_info = M('user as d ')
                ->field('d.id,d.invite_code,d.invite_type,d.invite_from_code,d.invite_name,d.mobile,d.cart_id')
                ->where($condition)
                ->select();
        if (!empty($user_info)) {
            foreach ($user_info as $key => $value) {
                $value['type'] = 2;
                array_push($driver_info, $value);
            }
        }
        $count = count($driver_info);
        $page = new \Think\Page($count, $size = 15);
        $fpage = $count > $size ? $page->Show() : '';
        
        $driver_info = array_slice($driver_info, ($p-1)*$size,$size,TRUE);//array_slice($driver_info,($p-1)*$size,$size,TRUE);
        $this->assign('keywords', $keywords);
    ...
    public function driver_invite() {
        $id = I('get.id');
        $type = I('get.type');
        
        if($type == 2){//乘客
            $invite_code = M('user')->where(['id'=>$id])->field('invite_code,invite_code_old')->find();
        }else{
            $invite_code = M('driver')->where(['id'=>$id])->field('invite_code,invite_code_old')->find();
        }
        $invite = M('invite')->where(['invite_code_first' => ['in',[$invite_code['invite_code'],$invite_code['invite_code_old']]]])->field('beinvite_code')->select();
        foreach ($invite as $kk => $vv) {
            $driver = M('driver')->where(['invite_code' => $vv['beinvite_code']])->field('mobile,invite_name,id,cart_id')->find();
            $user = M('user')->where(['invite_code' => $vv['beinvite_code']])->field('mobile,invite_name,id,cart_id')->find();
        ...
        $this->assign('count', count($driver_info));
        $this->assign('info', $driver_info);
        $this->display();
    }
    
    //推广管理
    public function extend_old() {
        $session = session('companyInfo');
        $keywords = I('get.keywords');
        if (!empty($keywords)) {
            $where['d.invite_name|d.mobile'] = array('like', "%$keywords%");
        ...
        foreach ($driver_info as $k => $v) {
            ...
            $invite = M('invite')->where(['invite_code_first' => $v['invite_code']])->field('beinvite_code')->select();
